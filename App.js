      ...
      RNMediaEditor.embedText(options)
        .then(res => {
          this.setState({
            asset: {
              ...this.state.asset,
              uri: 'data:image/jpeg;base64,' + res[1],
            },
          });
          console.log(res);
        })
  ...
  render() {
    return (
      <View style={styles.container}>
        <View style={{flex: 1}}>{this.renderMedia()}</View>
        <View style={styles.container}>
          <Button
            onPress={() => {
              this.onButtonPress('image');
            }}
            title="Pick Image"
          />
          <Button
            onPress={() => {
              this.onButtonPress('video');
            }}
            title="Pick Video"
          />
          <Button onPress={this.onEmbedButtonPress} title="Embed Text" />
          <Button onPress={this.log} title="Log" />
          {this.renderInput()}
        </View>
      </View>
    );
